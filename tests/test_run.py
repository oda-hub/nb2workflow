...
def test_nb(test_inrepo_notebook):
    import nb2workflow.nbadapter
    
    basename = os.path.basename(test_inrepo_notebook).replace(".ipynb","_output")
    ...
    r = nb2workflow.nbadapter.nbrun(test_inrepo_notebook, {})
...
def test_nb_fail(test_inrepo_notebook):
    import nb2workflow.nbadapter

    try:
        nb2workflow.nbadapter.nbrun(test_inrepo_notebook, dict(emin=-1))
    except Exception as e:
        print("got", e)
        assert os.path.exists(os.path.dirname(test_inrepo_notebook).replace(".ipynb","_output.ipynb"))
        assert os.path.exists(os.path.dirname(test_inrepo_notebook).replace(".ipynb","_exceptions.json"))
    else:
...
def test_nb_version(test_inrepo_notebook):
    import nb2workflow.nbadapter
    import rdflib
    ...
    nba = nb2workflow.nbadapter.NotebookAdapter(test_inrepo_notebook)
...
def test_nb_autocollect(test_inrepo_notebook):
    import nb2workflow.nbadapter
...
def test_nb_attach_file(test_inrepo_notebook, limit):
    import nb2workflow.nbadapter
