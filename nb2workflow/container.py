...
def prepare_image(repo_source, from_image, service=True, nb2w_path=None, runprefix=""):
    ...
    dockerfile.append("RUN {} touch /repo-hash-{}; pip install -r /repo/requirements.txt".format(runprefix, repo_hash))
...
def main():
    ...
    tempdir=prepare_image(repo_path,args.from_image, service=not args.job, nb2w_path=args.nb2wpath, runprefix=args.docker_run_prefix)
    ...
    if args.build:
        ...
        if args.job:
            ...
            for n, nba in find_notebooks(repo_path).items():
                nb2cwl_container(tag_image, nba.notebook_fn, n+".cwl", command=args.docker_command)
